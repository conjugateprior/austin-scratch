...
\author{Will Lowe\\Department of Political Science\\University of Maastricht}
...
Austin takes word frequency matrices in various formats.  Natively it
expects instances of the class \texttt{wfm}, a thin wrapper around a
simple matrix object. It expects you to use the helper functions
...
are columns.  The \texttt{docs} and \text{words} accessor functions
are there to remind you what the document names and words are.
...
\texttt{wfm} is an interface rather than a class, so anything
that can be indexed like a two dimensional matrix, e.g. a \texttt{data.frame}
or a \text{matrix} with a full complement of row and column names and 
\texttt{dimnames} set to `words' and `docs' will behave like a word
frequency matrix for all the functions in the package.  The function 
\texttt{is.wfm} can be used to confirm this.  Since they have no particular
class, we shall refer to objects for which \texttt{is.wfm} is true
simply as word frequency 
matrices.

If you use the \texttt{tm} package you probably have a
\texttt{TermDocumentMatrix} or \texttt{DocumentTermMatrix}.
Use \texttt{as.wfm} to convert these things into
\texttt{wfm} objects.

Once you have a word frequency matrix it can be treated like a regular
matrix.  In particular, you will want to use \texttt{subset} to extract 
documents or words in such a way as to keep \texttt{is.wfm} true of the results\footnote{This 
also has the happy consequence of allowing you to extract a single column from the matrix
as an appropriately labeled column vector rather than as a denatured numeric array, as is
R's curious default behaviour.}.  

The function 
\texttt{trim} remove low frequency words and words that occur in
only a few documents.  It can also be used to sample randomly from the set of words.  
This can be helpful to 
speed up analyses and check robustness to vocabulary choice.
...
Theory or generalized Latent Trait models with a Poisson link
\citep[e.g.][]{MoustakiKnott2000}.  

Austin
implements a version of Wordfish with faster convergence, superior
standard errors, and the possibility of out-of-sample predictions.
...
In Slapin and Proksch's version and the Austin implementation the
...
\begin{Scode}{echo=TRUE,fig=FALSE}
pdocs <- as.docword(dd$Y)[c(4,5),]
predict(wf, newdata=pdocs, se.fit=TRUE, interval='conf')
\end{Scode}
...
Positions for these documents are estimated by averaging the scores of
...
\begin{Scode}{echo=TRUE,fig=FALSE}
ref <- subset(lbg, select=1:5)
vir <- subset(lbg, select=6) 
\end{Scode}
