wordfish <- function(wfm,
    ...
    if (min(as.numeric(colSums(Y)))==0)
        stop("Some documents have no words in them")
    if (min(as.numeric(colSums(tY)))==0)
        stop("Some words do not appear in any document")
    ...
    if (!is.null(control$startparams)) {
        pars <- coef(control$startparams, form='poisson')
        params <- list(beta=pars$words$beta,
                       psi=pars$words$psi,
                       alpha=pars$docs$alpha,
                       theta=control$startparams$theta)
    } else {
...
sim.wordfish <- function(docs=10,
    ...
    Y <- data.frame(data)
    colnames(Y) <- paste("D", zfill(1:docs), sep="")
    rownames(Y) <- paste("W", zfill(1:vocab), sep="")

    val <- list(Y=wfm(Y, word.margin=1),
                theta=theta,
                doclen=colSums(Y),
                psi=b0,
                beta=b1)
    ...
    if (is(object, 'wordfish')){
        ...
        for (l in 1:L){
            ...
            newparams <- wordfish(newY, dir=m$dir,
                                  control=list(sigma=m$sigma, startparams=m, tol=m$tol, verbose=FALSE))
